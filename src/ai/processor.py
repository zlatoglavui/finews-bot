...
class AIProcessor:
    ...
    async def _call_gemini(self, prompt: str) -> str | None:
        ...
        # Достаём текст безопасно — разные версии SDK возвращают по-разному
        try:
            text = response.text
        except Exception:
            try:
                ...
                logger.warning(f"Не удалось извлечь текст из ответа Gemini: {e}")
                return None

        if not text or not text.strip():
            logger.warning("Gemini вернул пустой текст")
            return None
