...
class NewsPipeline:
    def __init__(self):
        ...
        self._tg_ok    = False  # флаг: Telegram доступен
    ...
    async def startup(self):
        ...
        if not self._tg_ok:
            logger.warning(
                "⚠️  Telegram недоступен — статьи будут собираться, "
                "но публикации не будет"
            )
    ...
    async def step_filter(self, articles: list[dict]) -> list[dict]:
        ...
        # Сохраняем отклонённые с нужным статусом (чтобы не перепроверять)
        for a in rejected:
    ...
    async def step_store_and_dedup(self, articles: list[dict]) -> list[dict]:
        ...
        # Загружаем существующие для сравнения
        ...
        existing_titles = [
            e["title"] for e in existing_pending + existing_processed
        ]
        ...
        for a in unique:
            ...
            if row_id:
                ...
                a["id"] = row_id  # нужен в следующих шагах
    ...
    async def step_publish(self, pairs: list[tuple[dict, str]]) -> int:
        if not self._tg_ok:
            ...
            # Помечаем как processed, чтобы при восстановлении не потерять
            for art, _ in pairs:
                if art.get("id"):
        ...
        for art, post in pairs:
            ...
            if msg_id:
                ...
                # Ошибка отправки — оставляем processed для повторной попытки
                if art.get("id"):
                    await self.db.update_article(art["id"], status="processed")
    ...
    async def run_once(self):
        ...
        try:
            ...
            unique = await self.step_store_and_dedup(passed)
            stats["new"] = len(unique)

            pairs = await self.step_ai(unique)
            stats["errors"] = len(unique) - len(pairs)
